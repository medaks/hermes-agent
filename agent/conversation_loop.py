...
def _o2_tool_round_observability(agent, s: _LoopState) -> None:
    """o2-only (default model): log the completed tool round and, every 31 tool
    calls since the last user message, inject a periodic nudge. Custom layer on
    the phase loop; never raises."""
    try:
        if getattr(agent, "model", "") != "o2":
            return
        _tool_names = [
            getattr(tc, "function", None).name
            for tc in (getattr(s.assistant_message, "tool_calls", None) or [])
            if getattr(tc, "function", None) is not None
        ]
        logger.info(
            "Tool calls: session=%s iteration=%d tools=%s",
            agent.session_id or "none", s.api_call_count, _tool_names,
        )
        # Periodic nudge: if tool calls since the last user message is a
        ...
        if agent._tool_calls_since_user % 31 == 0:
            s.messages.append({
                "role": "user",
                "content": "You have made many tool calls since the last user message. "
                           "If you are stuck, reconsider your approach. The context will "
                           "auto-compress when needed or you can decide to compress yourself.",
            })
        ...
        if _last_tool_results:
            logger.info(
                ...
                agent.session_id or "none", s.api_call_count, _last_tool_results,
            )
    except Exception:  # noqa: BLE001 - observability must never break the loop
        logger.debug("o2 tool-round observability skipped", exc_info=True)
...
def _run_conversation_turn(
    ...
    while (s.api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # o2-only: per-iteration logging for observability (custom; o2 is the
        # default model on this installation).
        if agent.model == "o2":
            logger.info(
                "Loop iteration: session=%s model=%s iteration=%d/%d "
                "messages_in=%d remaining_budget=%d",
                agent.session_id or "none", agent.model, s.api_call_count,
                agent.max_iterations, len(s.messages),
        ...
        try:
            ...
            if _ri.action == "continue":
                continue
            _ran_tool_round = bool(s.assistant_message.tool_calls)
            _v = _run_phase(
                run_tool_round if _ran_tool_round else finish_text_response, agent, s
            )
            if _v.action == "return":
                return _v.result
            if _v.action == "break":
                break
            if _ran_tool_round:
                _o2_tool_round_observability(agent, s)
