...
def _run_compress_context_tool(agent, args: dict, ctx: InlineToolContext) -> Any:
    """Run the custom compress_context tool against the live turn-local messages."""
...
INLINE_TOOL_EXECUTORS: Dict[str, InlineToolExecutor] = {
    ...
    # Agent-initiated context compression — must run through the live turn-local
    # messages list so the transcript actually shrinks (custom tool; upstream
    # has the compression machinery via agent._compress_context but no
    # model-callable tool).
    "compress_context": lambda agent, args, ctx: _run_compress_context_tool(
        agent, args, ctx
    ),
}
