...
def _tool_guidance_block(agent: Any) -> Optional[str]:
    ...
    tool_guidance = [
        ...
        SKILLS_GUIDANCE if names else None,
        _kanban_guidance,
    ]
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # MEMORY.md and USER.md are NOT injected via the built-in store (disabled —
    # the agent consults on-disk memory files via read_file). Instead the
    # memory index reference below points the agent at the richer per-domain
    # memory files on disk. The file path is stable across sessions
    # (cache-friendly); contents are read at session start so editing the .md
    # instantly updates the system prompt without code changes.
    import os as _os
