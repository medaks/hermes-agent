...
def _run_edge_tts(text: str, file_str: str, tts_config: Dict[str, Any]) -> None:
    ...
    try:
        ...
        import os as _os
        # Timeout enlarged and made configurable: on-device users prefer
        # completeness over speed. Override via HERMES_TTS_TIMEOUT (seconds);
        # default 600s (was 60s).
        _tts_timeout = int(_os.environ.get("HERMES_TTS_TIMEOUT", "600"))
        with ThreadPoolExecutor(max_workers=1) as pool:
            pool.submit(run).result(timeout=_tts_timeout)
    except RuntimeError:
        run()
