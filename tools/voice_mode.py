...
def detect_audio_environment() -> dict:
    ...
    # VM audio stack: the snd-aloop virtual card IS our audio hardware. The mic
    # and speakers live on the laptop; a UDP relay feeds/reads the loopback.
    # There is deliberately NO PulseAudio/PipeWire server here, so the SSH/container
    # checks below must not hard-block when the loopback card is present. Probe it
    # early so those blocks can defer.
    _loopback_devices = []
    try:
        _sd_env, _ = _import_audio()
        for _d in _sd_env.query_devices():
    ...
    # socket or forwarding env vars), which works fine over SSH (issue #35622).  The VM audio stack is the
    # exception: the snd-aloop virtual card is a real PortAudio device, so SSH without a sound server is fine.
...
class AudioRecorder(_RecorderBase):
    ...
    def stop(self) -> Optional[str]:
        """Stop recording and return the WAV path, or None if unusable. The underlying
        stream is kept alive for reuse (macOS CoreAudio: closing/re-opening hangs);
        on Linux/ALSA (the VM audio stack) it is released so the wake-word listener
        can reclaim substream 0 — the next recording re-creates it via _ensure_stream."""
        wav_path = None
        with self._lock:
            ...
            if self._peak_rms < SILENCE_RMS_THRESHOLD:
                ...
                return None
            wav_path = self._write_wav(audio_data, sample_rate=self._sample_rate)
        # Outside the lock: releasing the stream can block; never hold _lock.
        # Stream stays alive — no close needed.  (macOS CoreAudio:
        # closing/re-opening hangs; Linux/ALSA: must release so the wake-word
    ...
    def cancel(self) -> None:
        """Stop recording and discard all captured audio. The underlying stream is
        kept alive for reuse — EXCEPT on Linux/ALSA (see stop()): release the stream
        so the wake-word listener can re-arm on substream 0."""
        self._discard()
        if os.name != "nt" and sys.platform != "darwin":
...
def _play_wav_via_sounddevice(file_path: str) -> bool:
    ...
    try:
        ...
        # VM audio stack: resample to 16 kHz before playing. TTS files arrive at
        # the provider's native rate (Mistral: 24 kHz); if played at that rate,
        # the ALSA plug opens the snd-aloop pair at 24 kHz while the relay holds
        # the paired capture at 16 kHz — snd-aloop then delivers a clipped
        # half-rate stream and the wake word's 16 kHz re-arm fails with
        # paInvalidSampleRate. Linear resample to SAMPLE_RATE (16k) keeps every
        # path on one rate. (Only done when needed; 16 kHz audio passes through.)
