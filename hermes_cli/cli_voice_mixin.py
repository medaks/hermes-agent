...
class CLIVoiceMixin:
    ...
    def _voice_start_recording(self):
        ...
        if getattr(self, "_wake_word_active", False) and not getattr(self, "_wake_suspended", False):
            try:
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            mp3_path = os.path.join(out_dir, f"tts_{time.strftime('%Y%m%d_%H%M%S')}.wav")
    ...
    def _on_wake_word(self):
        ...
        with self._voice_lock:
            ...
            # also gets spoken output without a manual /voice tts first.
            try:
                _wake_vc = _config_section("voice")
                if isinstance(_wake_vc, dict) and _wake_vc.get("auto_tts", False):
                    self._voice_tts = True
            except Exception:
                pass
