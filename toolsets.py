...
TOOLSETS = {
    ...
    "web": _ts("Web research and content extraction tools", ["web_search", "web_extract", "compress_context"]),
    "search": _ts("Web search only (no content extraction/scraping)", ["web_search", "compress_context"]),
    "x_search": _ts(
    ...
    "vision": _ts("Image analysis and vision tools", ["vision_analyze", "compress_context"]),
    "video": _ts("Video analysis and understanding tools (opt-in, not in default toolset)", ["video_analyze"]),
    "image_gen": _ts("Creative generation tools (images)", ["image_generate", "compress_context"]),
    "video_gen": _ts(
    ...
    "terminal": _ts("Terminal/command execution and process management tools", ["terminal", "process_manage", "compress_context"]),
    "skills": _ts(
        "Access, create, edit, and manage skill documents with specialized "
        "instructions and knowledge",
        ["skills_list", "skill_view", "skill_manage", "compress_context"],
    ),
    ...
    "cronjob": _ts(
        ...
        ["cronjob_manage", "compress_context"],
    ),
    "file": _ts(
        "File manipulation tools: read, write, patch (with fuzzy matching), and "
        "search (content + files)",
        ["read_file", "write_file", "patch", "search_files", "compress_context"],
    ),
    "tts": _ts("Text-to-speech: convert text to audio with Edge TTS (free), ElevenLabs, OpenAI, or xAI", ["text_to_speech", "compress_context"]),
    "todo": _ts("Task planning and tracking for multi-step work", ["todo_list", "compress_context"]),
    # Memory tool disabled — the agent consults on-disk memory files via read_file
    # using the memory_index.md pointer in the system prompt. The toolset slot is
    # kept (legacy configs may enable it) and grants context compression instead.
    "memory": _ts("Persistent memory across sessions (personal notes + user profile) — tool disabled, agent uses read_file + memory_index.md", ["compress_context"]),
    "context_engine": _ts("Runtime tools exposed by the active context engine"),
    "session_search": _ts("Search and recall past conversations with summarization", ["session_search", "compress_context"]),
    ...
    "clarify": _ts("Ask the user clarifying questions (multiple-choice or open-ended)", ["clarify", "compress_context"]),
    "code_execution": _ts("Run Python scripts that call tools programmatically (reduces LLM round trips)", ["execute_code", "compress_context"]),
    "delegation": _ts("Spawn subagents with isolated context for complex subtasks", ["delegate_task", "compress_context"]),
    "homeassistant": _ts("Home Assistant smart home control and monitoring", _HA_TOOLS + ["compress_context"]),
    "kanban": _ts(
